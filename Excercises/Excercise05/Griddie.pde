...
class Griddie {
  ...
  void colorShift()
  {
     // CHANGED - Checks if milliseconds is greater than 2sec
     if (millis() > timeInterval)
    {
      //CHANGED - if it is than it changes colors 
      time = millis();
      fill(random(0,255),random(0,255),random(0,255), energy);
    }
  }
  ...
  //CHANGED - shifts to random colors based on timer   
  colorShift();   
  rect(x, y, size, size);
  } 
}